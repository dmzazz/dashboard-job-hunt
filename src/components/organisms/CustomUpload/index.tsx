...
const CustomUpload: FC<CustomUploadProps> = ({ form, name }) => {
  ...
  useEffect(() => {
    async function getImage() {
      const { publicUrl } = await supabaseGetPublicUrl(form.getValues(name), "company");
    ...
    <div className="inline-flex items-center gap-8">
      ...
      <div className="py-6 px-10 border-2 cursor-pointer border-bluePrimary border-dashed w-max rounded-sm" onClick={handleUploadFile}>
        ...
        <div className="text-center">
          <span className="text-bluePrimary font-medium">Click to replace</span> <span className="text-gray-500">or drag and drop</span>
        </div>
        <div className="text-gray-600 text-sm">PNG, JPG, JPEG (max. 400 x 400px)</div>
        <input ref={inputRef} type="file" className="hidden" onChange={handleFileChange} accept="image/png, image/jpeg, image/jpg" />
      </div>
    </div>
  );
};
